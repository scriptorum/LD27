...
class LevelService
{
	public static var FILE:String = "data.xml";
	public static var ASSETS_PATH:String = "xml/" + FILE;
#if (development && !flash)
	public static var SAVE_FOLDER:String = "/tmp/";
	...
	public static function readLevel(level:Int, throwIfMissing:Bool = true): Xml
	{
		for(levelXml in xml.elementsNamed("level"))
			if(Std.parseInt(levelXml.get("id")) == level)
				return levelXml;			
